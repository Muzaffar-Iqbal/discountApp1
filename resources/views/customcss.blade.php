                ...
                <div class="form-group">
                    <label> Select Product table Template </label>
                            <select class="form-control select-template" name="template">
								<option value="no" @if ($template == 'template1') selected @endif>Template 1 (Default)</option>
                                <option value="template1" @if ($template == 'template2') selected @endif >Template 2</option>
                                <option value="template2" @if ($template == 'template3') selected @endif >Template 3</option>
                                <option value="template3" @if ($template == 'template4') selected @endif >Template 4</option>
                           </select>
                    </div>
<hr>
                <div class="form-group">
                    ...
                    <textarea class="form-control" rows="10" id="css" placeholder=".custom-style" name="customCss">{{$css}}</textarea>
...
<div class="customCss"></div>
		...
		$( document ).ready(function() {
			if($('#customCssE').is(':checked')){
				$('#changeTable').removeAttr('class');
				$('#changeTable').addClass('design-table custom-style').addClass('design-no');
				var css="<style>"+$('#css').val()+"</style>";
				$('.customCss').append(css);
			}
		});
       ...
       $(document).on('change', '#customCssE', function(){
            if(this.checked){
				$('#changeTable').removeAttr('class');
				$('#changeTable').addClass('design-table custom-style').addClass('design-no');
				var css="<style>"+$('#css').val()+"</style>";
        ...
        $('.select-template').change(function(){
            var $val = $(this).val();
            $('#changeTable').removeAttr('class');
            $('#changeTable').addClass('design-table custom-style').addClass('design-'+$val);
        });
