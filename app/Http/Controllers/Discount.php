...
class Discount extends Controller
{
    ...
    public function allproducts()
    {
		...
		$selectedGroup = DB::table('groups')->get()->where('id', $groupid);
		$groups = DB::table('groups')->get()->where('shop', $session['shopId']);

        $arr = $groups->toArray();
		$arr1 = $selectedGroup->toArray();
        $data = array(
            'enable' => $on,
            'groups' => $arr,
			'selectedGroup' => $arr1
    ...
    public function applyBulkCollcts(Request $request)
    {
        ...
        return redirect()->action('Discount@collections');
    }
    ...
    public function deleteGroups(Request $request)
    {
        ...
        DB::table('products')->where('discount_grp', '=', $groupdid)->delete(); // delete the grp from DB
        DB::table('collection')->where('discount_grp', '=', $groupdid)->delete(); // delete the grp from DB
        $result = $shopify->call([
            'METHOD' => 'GET',
            'URL' => '/admin/metafields.json'
        ]);
